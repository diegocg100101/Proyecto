...
class XolobotDriver(Node):
    ...
    def __init__(self, idRobot):
        self.manejaUsuario = False

        # Identificador del Robot
        self.id = idRobot
        super().__init__('xolobot_driver_' + str(self.id))
        ...
        self.clientWaterPermission = self.create_client(Reload, 'permission')       
        while not self.clientWaterPermission.wait_for_service(timeout_sec=1.0):
            self.get_logger().info('Estación de riego no responde, esperando...')

        # Fuente para publicar comandos de velocidad a Xolobot.
        self.pubVelocities = self.create_publisher(Twist, '/model/arlo_xolobot' + str(self.id) + '/cmd_vel', 10)
        ...
        self.subsOdom = self.create_subscription(Odometry, '/model/arlo_xolobot' + str(self.id) + '/gz_odometry', self.updatePosition, 10)
        ...
        self.subsRiego = self.create_subscription(String, '/riego' + str(self.id), self.checkWatering, 10)

        # Suscripción al tópico /sol que le ordena al robot ir a la estación de iluminación.
        self.subsSun = self.create_subscription(String, '/sol' + str(self.id), self.checkSun, 10)
    ...
    def wandering(self):
        print("Xolo " + str(self.id) + ": Buscando mariposas 🦋🦋🦋...")
...
def main(args=None):
    ...
    if len(sys.argv) < 2:
        print("Uso: ros2 run nodo_xolobot_driver start_xolo_driver <robot_id>")
        rclpy.shutdown()
        return

    xolobot_drv = XolobotDriver(int(sys.argv[1]))
    xolobot_drv.drive()
    
    #xolobot_drv = XolobotDriver(2)
    #xolobot_drv.drive()
    
    #xolobot_drv = XolobotDriver(3)
    #xolobot_drv.drive()
